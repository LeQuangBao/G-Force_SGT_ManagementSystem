app.controller('intakeCtrl', function($scope, $http, $filter, $resource) {
    ...
    function GetListIntake() {
        $scope.list = [];
        var Intake = $resource(baseUrl + '/api/intake');
    ...
    // Nút thêm và đóng
    /*
	 * $scope.Them2 = function() { if (Check_Add()) { var startdate = new
	 * Date($scope.startdate); var enddate = new Date($scope.enddate);
	 * 
	 * var Intake = $resource('/api/intake'); // Call action method (save) on
	 * the class // Intake.save({ intakeId: $scope.intakeid, intakeName:
	 * $scope.name, startDate: $scope.startdate, endDate: $scope.enddate,
	 * active: $scope.active }) .$promise.then(function() { GetListIntake();
	 * $('#myModal_them').modal('hide'); addAlert(); }, function(response) {
	 * alertFailMessage(failMessage); }); } }
	 */
    ...
    $scope.GetIntake = function(x) {
        ...
        var Intake = $resource(baseUrl + '/api/intake/:id', {
            id: '@id'
        });
        Intake.get({
            id: x.id
