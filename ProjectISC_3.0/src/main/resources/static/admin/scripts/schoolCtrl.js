app.controller('schoolCtrl', function($scope, $http, $filter) {
	var baseUrl ="http://localhost:8080";
       ...
       .then(function (result) {	
		 ...
		 * 
		 * var school_id = document.getElementById("schoolID").value; var
		 * school_name = document.getElementById("schoolName").value; var
		 * address=document.getElementById("address").value; var contact =
		 * document.getElementById("contact").value; var
		 * active1=document.getElementById("actived"); var active=$scope.active;
		 * 
		 * $http({ method: "POST", url: "/api/school", data: { schoolId:
		 * school_id, schoolName: school_name, address : address,
		 * contact:contact, active:active
		 *  },
		 * 
		 * dataType: "json" }) .then(function (result) { if (result.status ==
		 * 201) {
		 * 
		 * $("#myModal").modal("hide"); getAllSchools(); addAlert();
		 * $scope.ResetForm_Add(); }
		 *  }, function(response) { alertFailMessage("Oops! Something went
		 * wrong, please check your input again."); }); }
		 */
...
// }, alertDuration);
        ...
        $scope.editSchool = function (data) {
        	$http.get(baseUrl + "/api/school/"+data.id)
            .then(function (response) {
         ...
         $scope.delete=function()
        {
            $http({
               method: "DELETE",
              url: "http://localhost:8080/api/school/" + $scope.school_delete.id,
              dataType: "json",
            })
               .then(function (result) {
                  if (result.status == 202) {
